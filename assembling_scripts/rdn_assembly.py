...
conv_init = keras.initializers.Constant(value=0.2)

def first_type_layer(input_value, size):
    convolution_layer = Conv2D(size, kernel_size=(3, 3), padding='same',
...
def second_type_layer(input_value, size):
    ...
    convolution_layer = Conv2D(size, kernel_size=(3, 3), padding='same',
                               kernel_initializer='random_normal',
                               use_bias=True, bias_initializer=conv_init,
                               data_format="channels_first")(first_sublayer)
    batch_normalized_layer = BatchNormalization()(convolution_layer)
...
def third_type_layer(input_value, size):
    ...
    side_value = Conv2D(size, data_format="channels_first",
...
def fourth_type_layer(input_value, size):
    first_sublayer = first_type_layer(input_value, size)
    convolution_layer = Conv2D(size, data_format="channels_first", kernel_size=(3, 3),
                               padding='same',
                               use_bias=True, bias_initializer=conv_init,
                               kernel_initializer='random_normal')(first_sublayer)
    batch_normalized_layer = BatchNormalization()(convolution_layer)
